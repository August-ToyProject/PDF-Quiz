...
export default function AnswerSave({ onClose }: { onClose: () => void }) {
    ...
    const { quizData, elapsedTime, userAnswers, timeLimitHour, timeLimitMinute } = useQuizContext();

    const handleSave = async () => {
        try {
            const spentTime = (elapsedTime?.hours || 0) * 3600 + (elapsedTime?.minutes || 0) * 60 + (elapsedTime?.seconds || 0);
            ...
            const saveData = {
                ...
                exam: quizData.map((item, index) => ({
                    ...
                    correct: Object.keys(item.answer)[0] === userAnswers[index]?.[0]?.toString(),
                })),
            };

            console.log("서버에 요청할 데이터:", saveData);
    
            await saveQuizData(saveData.title, saveData.setTime, saveData.spentTime, saveData.exam);
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg">
                ...
                <div className="flex justify-center">
                    <button className="bg-blue-600 text-white font-bold py-2 px-4 rounded-full mr-2" onClick={handleSave}>
                        확인
                    </button>
                </div>
            </div>
