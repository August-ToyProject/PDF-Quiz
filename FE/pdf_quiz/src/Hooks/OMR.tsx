...
const OMR = () => {
            ...
            <div key={problemIndex} className="flex items-center mb-2">
              ...
              <div className="flex flex-row items-center justify-center">
                {Array.from({ length: options }).map((_, optionIndex) => (
                    <button className={`w-10 h-10 mr-1/2 border-0 flex items-center justify-center rounded-full cursor-pointer max-lg:w-7 max-lg:h-8 ${selectedAnswers[problemIndex] === optionIndex+1 ? 'bg-green-400' : 'bg-white'}`}
                    key={optionIndex} 
    ...
    return (
        <div className="max-lg:w-[25%] max-lg:text-sm">
            {OMRRendering()}
