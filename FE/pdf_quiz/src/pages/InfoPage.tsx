...
export default function InfoPage() {
  ...
  const [errors, setErrors] = useState<
    Partial<UserInfo & { password: string; passwordConfirm: string }>
  >({});
  const commonClassNames =
    "bg-white rounded-lg border border-gray-300 p-2 m-2 w-full";
  ...
  const handleInfoSave = async () => {
    ...
    if (userData) {
      ...
      promises.push(
        updateInfo(userData).catch((error) => {
      ...
      if (password && passwordConfirm) {
        promises.push(
          updatePassword(userData.email, { password, passwordConfirm }).catch(
            (error) => {
      ...
      <div className="bg-white p-4 w-80 flex flex-col justify-start min-h-[400px]">
        <div className="flex justify-center w-full mb-4">
          <button
            className={`bg-white px-4 py-2 rounded-none w-1/2 focus:outline-none ${
              activeButton === "info"
                ? "border-b-4 border-t-0 border-l-0 border-r-0 border-blue-600 text-blue-600"
        ...
        {activeButton === "info" ? (
          ...
          <div className="flex flex-col items-start w-full">
            ...
            />
            {errors.username && (
              <p className="ml-4 text-red-500 text-sm">{errors.username}</p>
            )}
            <h2 className="text-sm text-gray-600 ml-4">닉네임</h2>
            <input
              type="text"
            ...
            />
            {errors.nickname && (
              <p className="ml-4 text-red-500 text-sm">{errors.nickname}</p>
            )}
            ...
            />
            {errors.password && (
              <p className="ml-4 text-red-500 text-sm">{errors.password}</p>
            )}
